...
| 5.1 | Commit history — 367 commits | «» |
...
**Monetary values were migrated to integer cents.** The original schema used
`decimal(10,2)`. §2.4.2 explains the failure this caused.
...
wrote cents into them, while the client priced in dollars and rendered
`toFixed(2)`. A **$4.99 print was stored as `299.00` and displayed as $299.00** —
a hundredfold error in the direction that overcharges the customer.

**The resolution.** Integer cents everywhere. Migration `00017` renamed
`total_amount → total_cents` and `unit_price → unit_price_cents`, retyping both to
`integer` using `ROUND(...)` rather than truncation. Conversion to dollars occurs
...
A single shared catalogue defines the seven product types, their integer-cent
...
| **Order placement** | **201**, `total_cents: 998` for 2 × 499 — integer cents |
...
| Commits | 367 |
| Contributors | 4 |
| Period | 1 February – 9 August 2026 |
...
and a PostgreSQL database of nine domain tables across 20 migrations, with
...
5. **Add a Redis health check and timeout**, closing the defect in §6.3(7).
...
10. Data-retention policy and parent-initiated deletion, appropriate to a
